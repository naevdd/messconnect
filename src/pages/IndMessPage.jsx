...
function IndMessPage() {
  ...
  const { messname } = useParams(); // using messname from the route

  useEffect(() => {
    axios
      .get(`http://localhost:5000/indmess/${encodeURIComponent(messname)}`)
      .then((response) => {
          ...
          >
            ...
            <div className="flex flex-col w-1/2 mr-5">
              ...
              <div className="h-full w-full rounded-xl">
                <img
                  src={messData.image || "default-image-path.jpg"}
                  alt={messData.messname || "Mess Name"}
                />
              </div>
            ...
            <div className="flex flex-col w-full md:w-1/2 rounded-xl">
              <div className="bg-yellow-300 p-5 rounded-xl">
                ...
                <ul className="space-y-6 text-2xl text-left">
                  <li>
                    <strong>Timings:</strong> {messData.time}
                  </li>
                  <li>
                    <strong>Locations Served:</strong> {messData.location}
                  </li>
                  <li>
                    <strong>Price:</strong> {messData.price}
                  </li>
                  <li>
                    <strong>Food for the Day:</strong>{" "}
                    {Array.isArray(messData.food) ? (
                      <ul className='list-disc list-inside'>
                        {messData.food.map((meal, index) => (
                          <li className="text-xl" key={index}>
                            <strong>{meal.type}:</strong> {meal.items.join(", ")}
                          </li>
                        ))}
                      </ul>
                    ) : (
